...
from .preprocess.audio import normalize, track_id_for
...
def analyze_file(file_path_str: str):
    """Decode + extract features only (no DB), so it can run in a worker process.

    Returns (file_path, track_id, wav_path, TrackFeatures) or None on failure.
    ...
    try:
        ...
        return (str(file_path.resolve()), track_id_for(file_path), str(wav), feats)
...
class MusicLibrary:
    ...
    def _store(self, result, embedding_model: str) -> str | None:
        if result is None:
            return None
        file_path, tid, wav_path, feats = result
        try:
    ...
    def index(self, root: str | Path, recursive: bool = True, workers: int = 1) -> dict:
        ...
        if workers and workers > 1 and len(paths) > 1:
            ...
            with ProcessPoolExecutor(max_workers=min(int(workers), len(paths))) as ex:
                results = list(ex.map(analyze_file, paths, chunksize=1))
        else:
        ...
        summary = {"found": len(files), "indexed": len(ok), "failed": len(failed), "failed_paths": failed}
