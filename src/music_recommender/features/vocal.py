...
ship here, so this is a low-confidence HEURISTIC:
  ...
  - pitch statistics from pyin restricted to the vocal band
...
def vocal_features(y: np.ndarray, sr: int, max_seconds: float = 20.0) -> dict:
    import librosa
    ...
    if has_vocal:
        f0, voiced, _ = librosa.pyin(seg, sr=sr, fmin=VOCAL_FMIN, fmax=VOCAL_FMAX, frame_length=2048, hop_length=256)
